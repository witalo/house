...
$(document).ready(function () {
    new Autocomplete('#autocomplete-product', {
        ...
        onSubmit: result => {
            if (result) {
                AddRowDetail(0, 0, result.pk, result.name, 1, result.price, result.stock.id, result.stock.quantity, '00:00')
                $('#search-product').val('')
            }
        }
    })
...
async function AddRowDetail(i, pk, product, name, quantity, price, store, stock, time) {
    ...
    let amount = parseFloat(quantity * price)
    $('tbody#order_detail').append(
        '<tr class="p-0"  pk="' + pk + '" i="' + i + '" store="' + store + '" product="' + product + '" time="' + time + '">' +
        ...
        '<td class="align-middle item-price p-1 text-right">' + price.toFixed(2) + '</td>' +
        // '<td class="align-middle item-price">' + '<input type="number" step="0.01" min="0.00" placeholder="0.00" disabled class="form-control form-control-sm input-price text-right w-100" value="' + price + '"' + '</td>' +
        '<td class="align-middle mailbox-date item-amount p-1 text-right">' + amount.toFixed(2) + '</td>' +
...
function CountRow() {
    ...
    $('tbody#order_detail tr').each(function () {
        ...
        if (typeof product === 'number') {
            productValue = product; // Si product es un número, asigna directamente el valor
            $(this).find('td.item-delete button').attr('onclick', 'DeleteRowDetail(' + index + ', ' + pk + ', ' + productValue + ')')
        } else {
            productValue = "'" + product + "'"; // Si product es 'R', encierra el valor entre comillas simples
            $(this).find('td.item-delete button').attr('onclick', 'DeleteRowDetail(' + index + ', ' + pk + ', ' + productValue + ')')
        }
        index++;
    });
...
$(document).on('keyup change', 'tbody#order_detail tr td.item-quantity div.input-group input.input-quantity', function () {
    ...
    let input_price = row.find('td.item-price')
    let quantity = input.val()
    // let price = input_price.val()
    let price = input_price.text()
...
function CreateOrder() {
    ...
    // let init = $('#init').val()
    let current = fechaHoraFormateada
...
function AddPerson(v) {
    if (parseFloat(v) > 0) {
        if ($("tbody#order_detail tr[product=A]").length > 0) {
            toastr.info('El detalle ya se encuentra añadido')
            return false
        } else {
            AddRowDetail(0, 0, "A", 'PERSONA ADICIONAL', 1, v, 0, 1, '00:00')
        }
...
function AddNewPayment() {
    ...
    minutos = (minutos < 10) ? '0' + minutos : minutos;
    // Construye la cadena de fecha y hora en el formato deseado
    var fechaHoraFormateada = `${dia}/${mes}/${año} ${horas}:${minutos} ${periodo}`;
